...
# ============================================================
# 17. VERSION 2: MORTGAGE-PRODUCT CONTROLS
# ============================================================
...
# ============================================================
# 18. VERSION 2: LENDER FIXED EFFECTS
# ============================================================
...
# Sensitivity check excluding singleton lenders.
...
# ============================================================
# 19. VERSION 2: TRACT CHARACTERISTICS
# ============================================================
...
# ============================================================
# 20. VERSION 2: CENSUS-TRACT FIXED-EFFECTS STRESS TEST
# ============================================================
...
# ============================================================
# 21. LINKAGE-SELECTION SENSITIVITY
# ============================================================
...
selection_model_df <- hmda_selection %>%
  mutate(
    ...
    log_property_value =
      log(property_value_num),
    log_income =
      log(if_else(income_num > 0, income_num, NA_real_)),
    loan_type_factor =
      factor(loan_type),
    conforming_factor =
      factor(conforming_loan_limit),
...
# Assign fitted probabilities only to rows actually used by glm().
...
property_ipw <- property_complete %>%
  left_join(selection_weights, by = "hmda_id")
...
ipw_geo_cluster <- lmtest::coeftest(
  model_v2_geo_ipw,
  vcov. = sandwich::vcovCL(
    model_v2_geo_ipw,
    cluster = ~ lei,
...
# ============================================================
# 22. VERSION 2 RESULTS TABLE
# ============================================================

extract_rate_row <- function(model, test_table, model_name) {
  beta <- unname(coef(model)["interest_rate_num"])
  tibble(
    model = model_name,
    n = nobs(model),
    interest_beta = beta,
    robust_se = unname(test_table["interest_rate_num", "Std. Error"]),
    p_value = unname(test_table["interest_rate_num", "Pr(>|t|)"]),
    exact_percent_association = 100 * (exp(beta) - 1),
    adjusted_r2 = summary(model)$adj.r.squared
  )
}

version2_results <- bind_rows(
  extract_rate_row(
    model_linked_base, base_hc3,
    "A: HMDA controls"
  ),
  extract_rate_row(
    model_linked_property, property_hc3,
    "B: + SDAT property controls"
  ),
  extract_rate_row(
    model_v2_loan, v2_loan_hc3,
    "C: + loan type and conforming status"
  ),
  extract_rate_row(
    model_v2_lender, v2_lender_cluster,
    "D: + lender fixed effects"
  ),
  extract_rate_row(
    model_v2_geo, v2_geo_cluster,
    "E: + tract characteristics"
  ),
  extract_rate_row(
    model_v2_tract, v2_tract_cluster,
    "F: census-tract fixed-effects stress test"
  ),
  extract_rate_row(
    model_v2_geo_ipw, ipw_geo_cluster,
    "E-IPW: stabilized linkage-weighted Model E"
  )
)

print(version2_results, width = Inf)

write_csv(
  version2_results,
  "Paper1_QREF_Version2_Model_Comparison.csv"
)

write_csv(
  selection_summary,
  "Paper1_QREF_Linkage_Selection_Summary.csv"
)

weight_diagnostics <- property_ipw %>%
  filter(!is.na(ipw_stabilized)) %>%
  summarise(
    n = n(),
    min_weight = min(ipw_stabilized),
    median_weight = median(ipw_stabilized),
    p95 = quantile(ipw_stabilized, 0.95),
    p99 = quantile(ipw_stabilized, 0.99),
    max_weight = max(ipw_stabilized),
    effective_n =
      sum(ipw_stabilized)^2 / sum(ipw_stabilized^2)
  )

write_csv(
  weight_diagnostics,
  "Paper1_QREF_IPW_Weight_Diagnostics.csv"
)


# ============================================================
# 23. VERSION 2 REPRODUCTION CHECKS
# ============================================================
#
# These tolerances check the manuscript values without forcing
# equality to rounded printed coefficients.
# ============================================================
...
stopifnot(abs(coef(model_linked_base)["interest_rate_num"] - (-0.0809)) < 0.0001)
stopifnot(abs(coef(model_linked_property)["interest_rate_num"] - (-0.0405)) < 0.0001)
stopifnot(abs(coef(model_v2_loan)["interest_rate_num"] - (-0.04833179)) < 1e-6)
stopifnot(abs(coef(model_v2_lender)["interest_rate_num"] - (-0.04714448)) < 1e-6)
stopifnot(abs(coef(model_v2_geo)["interest_rate_num"] - (-0.04265044)) < 1e-6)
stopifnot(abs(coef(model_v2_tract)["interest_rate_num"] - (-0.03561263)) < 1e-6)
stopifnot(abs(coef(model_v2_geo_ipw)["interest_rate_num"] - (-0.03739838)) < 1e-6)

cat("\nQREF VERSION 2 REPRODUCTION COMPLETE\n")
print(version2_results, width = Inf)
print(weight_diagnostics, width = Inf)
cat("\nSESSION INFORMATION\n")
print(sessionInfo())
